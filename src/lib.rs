...
pub fn run() {
	...
	event_loop.run(move |event, _, control_flow| match event {
		...
		Event::MainEventsCleared => {
			...
			// Recieve chunk blocks generated by workers.
			game.worker_tasks.retain_mut(|worker_task| {
				let chunk_coords_and_result_opt = match worker_task {
				...
				if let Some((chunk_coords, chunk)) = chunk_coords_and_result_opt {
			...
			if game.enable_world_generation {
				...
				for neighbor_chunk_coords in iter_3d_cube_center_radius(player_chunk_coords, 4) {
					let was_generated = game.chunk_grid.map.contains_key(&neighbor_chunk_coords);
					let is_being_generated =
						game
							.worker_tasks
							.iter()
							.any(|worker_task| match worker_task {
								WorkerTask::FullChunkWithoutMesh(chunk_coords, ..) => {
									*chunk_coords == neighbor_chunk_coords
								},
							});
					if (!was_generated) && (!is_being_generated) && game.worker_tasks.len() < 12 {
						let chunk_coords = neighbor_chunk_coords;
						let (sender, receiver) = std::sync::mpsc::channel();
						game
							.worker_tasks
							.push(WorkerTask::FullChunkWithoutMesh(chunk_coords, receiver));
						...
						std::thread::spawn(move || {
							...
							std::thread::sleep(std::time::Duration::from_secs_f32(
								rand::thread_rng().gen_range(0.3..4.0),
							));
